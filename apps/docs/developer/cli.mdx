...
Its job is to be the installed user-facing shell around the local runtime, not the place where product logic is implemented.

The main interactive entrypoints are foreground session commands such as `executor web` and `executor mcp`, plus `executor mcp --stdio` for local process-spawned MCP integrations.

## Responsibilities

The CLI is responsible for:

- starting foreground web and MCP sessions
- optionally starting and stopping the detached local daemon for compatibility
...
1. starts a foreground local session or ensures the detached daemon is available
